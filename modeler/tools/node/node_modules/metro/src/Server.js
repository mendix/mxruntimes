...
class Server {
  constructor(config) {
    ...
    const processFileChange = _ref2 => {
      let type = _ref2.type,
        filePath = _ref2.filePath;
      return this.onFileChange(type, filePath);
    };
    ...
    this._bundler = new Bundler(config); // changes to the haste map can affect resolution of files in the bundle
    this._bundler.getDependencyGraph().then(dependencyGraph => {
      dependencyGraph.getWatcher().on("change", _ref3 => {
        let eventsQueue = _ref3.eventsQueue;
        return eventsQueue.forEach(processFileChange);
      });
    });
  ...
  buildGraph(entryFiles, options) {
    var _this3 = this;
    return _asyncToGenerator(function*() {
      entryFiles = entryFiles.map(function(entryFile) {
        return getEntryAbsolutePath(_this3._config, entryFile);
      });
      const graph = yield _this3._deltaBundler.buildGraph(entryFiles, {
      ...
      _this3._config.serializer.experimentalSerializerHook(graph, {
  ...
  getRamBundleInfo(options) {
    var _this4 = this;
    return _asyncToGenerator(function*() {
      const graphInfo = yield _this4._buildGraph(options);
      const entryPoint = getEntryAbsolutePath(
        _this4._config,
        options.entryFile
      );
      return yield getRamBundleInfo(
        ...
        graphInfo.graph,
        {
          ...
          excludeSource: options.excludeSource,
          getRunModuleStatement:
            _this4._config.serializer.getRunModuleStatement,
          getTransformOptions: _this4._config.transformer.getTransformOptions,
          platform: options.platform,
          projectRoot: _this4._config.projectRoot,
          runBeforeMainModule: _this4._config.serializer.getModulesRunBeforeMainModule(
            path.relative(_this4._config.projectRoot, entryPoint)
          ),
          runModule: options.runModule,
          sourceMapUrl: options.sourceMapUrl,
          inlineSourceMap: options.inlineSourceMap
  ...
  getAssets(options) {
    var _this5 = this;
    return _asyncToGenerator(function*() {
      var _ref4 = yield _this5._buildGraph(options);
      const graph = _ref4.graph;
      return yield getAssets(graph, {
        processModuleFilter: _this5._config.serializer.processModuleFilter,
        assetPlugins: _this5._config.transformer.assetPlugins,
        platform: options.platform,
        projectRoot: _this5._config.projectRoot
      });
    })();
  }
  getOrderedDependencyPaths(options) {
    var _this6 = this;
    return _asyncToGenerator(function*() {
      options = _extends({}, Server.DEFAULT_BUNDLE_OPTIONS, options, {
        bundleType: "bundle"
      });
      var _ref5 = yield _this6._buildGraph(options);
      const prepend = _ref5.prepend,
        graph = _ref5.graph;
      const platform =
        options.platform ||
        parsePlatformFilePath(options.entryFile, _this6._platforms).platform;
      return yield getAllFiles(prepend, graph, {
        platform,
        processModuleFilter: _this6._config.serializer.processModuleFilter
      });
    })();
  }
  _buildGraph(options) {
    ...
    return _asyncToGenerator(function*() {
      ...
      const graph = yield _this7._deltaBundler.buildGraph([entryPoint], {
        resolve: yield transformHelpers.getResolveDependencyFn(
          _this7._bundler,
          options.platform
        ),
        transform: yield transformHelpers.getTransformFn(
          [entryPoint],
          _this7._bundler,
          _this7._deltaBundler,
          _this7._config,
          crawlingOptions
        ),
        onProgress: options.onProgress
      });
      _this7._config.serializer.experimentalSerializerHook(graph, {
        modified: graph.dependencies,
        deleted: new Set(),
        reset: true
      });
  ...
  _getGraphInfo(options, _ref6) {
    var _this8 = this;
    let rebuild = _ref6.rebuild;
    return _asyncToGenerator(function*() {
      const id = _this8._optionsHash(options);
      let graphPromise = _this8._graphs.get(id);
      let graphInfo;
      let numModifiedFiles = 0;
      if (!graphPromise) {
        graphPromise = _this8._buildGraph(options);
        _this8._graphs.set(id, graphPromise);
        ...
        graphInfo = yield graphPromise;
        if (rebuild) {
          const delta = yield _this8._deltaBundler.getDelta(graphInfo.graph, {
            reset: false
          });
          numModifiedFiles = delta.modified.size;
          if (numModifiedFiles > 0) {
            graphInfo.lastModified = new Date();
            _this8._config.serializer.experimentalSerializerHook(
              graphInfo.graph,
              delta
            );
          }
  ...
  _getDeltaInfo(options) {
    var _this9 = this;
    return _asyncToGenerator(function*() {
      const id = _this9._optionsHash(options);
      let graphPromise = _this9._deltaGraphs.get(id);
      let graphInfo;
      let delta;
      if (!graphPromise) {
        graphPromise = _this9._buildGraph(options);
        _this9._deltaGraphs.set(id, graphPromise);
        graphInfo = yield graphPromise;
        delta = {
        ...
        graphInfo = yield graphPromise;
        delta = yield _this9._deltaBundler.getDelta(graphInfo.graph, {
          reset: graphInfo.sequenceId !== options.deltaBundleId
        });
        _this9._config.serializer.experimentalSerializerHook(
          graphInfo.graph,
          delta
        ...
        graphInfo = _extends({}, graphInfo, {
          sequenceId: crypto.randomBytes(8).toString("hex")
        });
        _this9._deltaGraphs.set(id, graphInfo);
      }
      return _extends({}, graphInfo, { delta });
    })();
  }
  ...
  _processSingleAssetRequest(req, res) {
    ...
    return _asyncToGenerator(function*() {
      ...
      try {
        ...
        res.end(_this10._rangeRequestMiddleware(req, res, data, assetPath));
  ...
  _processDeltaRequest(req, res) {
    var _this11 = this;
    return _asyncToGenerator(function*() {
      const mres = MultipartResponse.wrap(req, res);
      var _prepareDeltaBundler = _this11._prepareDeltaBundler(req, mres);
      const options = _prepareDeltaBundler.options,
        buildID = _prepareDeltaBundler.buildID;
      ...
      try {
        ...
        var _ref7 = yield _this11._getDeltaInfo(options);
        delta = _ref7.delta;
        graph = _ref7.graph;
        prepend = _ref7.prepend;
        ...
        output = {
          bundle: deltaJSBundle(
            ...
            {
              processModuleFilter:
                _this11._config.serializer.processModuleFilter,
              createModuleId: _this11._createModuleId,
              dev: options.dev,
              getRunModuleStatement:
                _this11._config.serializer.getRunModuleStatement,
      ...
      mres.setHeader(
        ...
        String(output.numModifiedFiles)
      );
      mres.setHeader(DELTA_ID_HEADER, String(sequenceId));
      mres.setHeader("Content-Type", "application/json");
      mres.setHeader(
        "Content-Length",
        String(Buffer.byteLength(output.bundle))
      );
      ...
      log(
        _extends({}, createActionEndEntry(requestingBundleLogEntry), {
  ...
  _processBundleRequest(req, res) {
    var _this12 = this;
    return _asyncToGenerator(function*() {
      const mres = MultipartResponse.wrap(req, res);
      var _prepareDeltaBundler2 = _this12._prepareDeltaBundler(req, mres);
      const options = _prepareDeltaBundler2.options,
        buildID = _prepareDeltaBundler2.buildID;
      ...
      try {
        var _ref8 = yield _this12._getGraphInfo(options, { rebuild: true });
        const graph = _ref8.graph,
          prepend = _ref8.prepend,
          lastModified = _ref8.lastModified,
          numModifiedFiles = _ref8.numModifiedFiles;
        ...
        result = {
          bundle: plainJSBundle(options.entryFile, prepend, graph, {
            ...
            getRunModuleStatement:
              _this12._config.serializer.getRunModuleStatement,
            dev: options.dev,
  ...
  _processSourceMapRequest(req, res) {
    var _this13 = this;
    return _asyncToGenerator(function*() {
      const mres = MultipartResponse.wrap(req, res);
      var _prepareDeltaBundler3 = _this13._prepareDeltaBundler(req, mres);
      const options = _prepareDeltaBundler3.options,
        buildID = _prepareDeltaBundler3.buildID;
      ...
      try {
        var _ref9 = yield _this13._getGraphInfo(options, {
          rebuild: false
        });
        const graph = _ref9.graph,
      ...
      log(
        createActionEndEntry(
          _extends({}, requestingBundleLogEntry, {
            bundler: "delta"
          })
        )
      );
  ...
  _processAssetsRequest(req, res) {
    ...
    return _asyncToGenerator(function*() {
      const mres = MultipartResponse.wrap(req, res);
      var _prepareDeltaBundler4 = _this14._prepareDeltaBundler(req, mres);
      const options = _prepareDeltaBundler4.options,
        buildID = _prepareDeltaBundler4.buildID;
      ...
      log(
        createActionEndEntry(
          _extends({}, requestingAssetsLogEntry, {
            bundler: "delta"
          })
        )
      );
  ...
  _sourceMapForURL(reqUrl) {
    var _this15 = this;
    return _asyncToGenerator(function*() {
      const options = _this15._getOptionsFromUrl(reqUrl);
      var _ref10 = yield _this15._getGraphInfo(options, {
        rebuild: false
      });
      const graph = _ref10.graph,
  ...
  _handleError(res, options, error, buildID) {
